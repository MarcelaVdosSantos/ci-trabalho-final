...
- **Objetivo:** Agir como um _Smoke Test_ de ciclo contínuo para verificar a estabilidade do ambiente após atualizações de fluxo.
...
- **Fluxo Sequencial de Jobs:**
  ...
  2.  **Job Testes (`testes`):** Executa o ambiente e os navegadores do Playwright para rodar a suíte Mocha. Através da centralização de logs no `config-reporters.json`, quatro relatórios são gerados simultaneamente:
      ...
      - **CTRF (`ctrf-report.json`):** Painel interativo com taxas de sucesso, estabilidade (_flaky tests_) e testes mais lentos renderizado no sumário do GitHub.
...
_Obs.: Se o Prettier acusar desalinhamento ou problemas de code style no seu código ou arquivos YAML, use `npx prettier --write .` para corrigi-los instantaneamente de forma automática._
